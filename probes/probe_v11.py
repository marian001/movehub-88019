#!/usr/bin/env python3
"""M0b probe v11 — proper VM handshake then flags/tail/mode sweep.
Per 42176-controller: VM must be ARMED (opcode 0x04) or it ACKs and ignores.
Handshake: SUBSCRIBE (input format on 0x36 m0) -> STATE_REQUEST ->
  if stopped: STOP -> START -> ARM ; if running: ARM.
Then: calibration, flags sweep, tail sweep, VM modes, light demo."""
...
LOG_PATH = "probe_m0_log_v11.json"
...
def on_notify(_c, data):
    b = bytes(data)
    note("notify", b)
    # track VM state from port value msgs for 0x36: 0c 00 45 36 <a> <b>
    if len(b) >= 6 and b[2] == 0x45 and b[3] == 0x36:
        vm_state["raw"] = (b[4], b[5])
    notify_event.set()
...
async def main():
    ...
    try:
        ...
        await w(client, LED(0x06), "led_green_sanity", wait=0.3)
        await w(client, VM_SUBSCRIBE, "vm_subscribe", wait=0.4)
        await w(client, VM_STATE_REQ, "vm_state_req", wait=0.8)
        note("vm_state_seen", str(vm_state["raw"]))
        # arm always (harmless if already armed; needed if running-but-not-armed)
        await w(client, VM_ARM, "vm_arm", wait=0.5)
        # if state looked stopped (raw[1]==0), do full restart
        if vm_state["raw"] and vm_state["raw"][1] == 0:
            note("phase", "VM looks STOPPED -> full restart")
            await w(client, VM_STOP, "vm_stop", wait=0.6)
            await w(client, VM_START, "vm_start", wait=1.0)
            await w(client, VM_ARM, "vm_arm2", wait=0.6)
        await w(client, VM_STATE_REQ, "vm_state_req2", wait=0.6)
        note("vm_state_after", str(vm_state["raw"]))

        # ---------- calibration (with working VM now) ----------
        await w(client, vm(0, 0, 0x10), "calib1", wait=1.5)
        ...
        # ---------- VM mode sweep ----------
        note("phase", "VM mode sweep (mode byte)")
        for mode in (0x00, 0x01, 0x02, 0x04, 0x05):
            await w(client, vm(0, 0, 0x00, mode=mode), f"vmode_{mode:02x}", wait=1.6)

        # ---------- light demo (prove VM armed works) ----------
        note("phase", "demo: lights blink x3")
        for i in range(3):
