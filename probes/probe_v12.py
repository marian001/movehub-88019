#!/usr/bin/env python3
"""M0b probe v12 — cold-start + correct VM state machine + drive tests.
CRITICAL: power-cycle the hub FIRST (hold button ~5s until LED off, then
press once), because sessions after a crash/unclean disconnect leave the
VM refusing drive frames (ERR 0x05). Correct state decoding:
  (01,00)=RUNNING (02,00)=STOPPING (00,01)=STOPPED (03,01)=EXECUTING
If STOPPED -> STOP -> START -> ARM. If RUNNING -> ARM only.
Then: calibration -> flags sweep -> tail sweep -> light demo."""
...
LOG_PATH = "probe_m0_log_v12.json"
...
async def find_hub(timeout_s=600):
    ...
    while loop.time() < deadline:
        ...
        if dev:
            ...
            return dev
        print("  ...rescanning (power-cycle hub: hold ~5s to OFF, then press ON)", flush=True)
    return None
...
async def main():
    ...
    try:
        ...
        # ---------- VM handshake (correct state machine) ----------
        ...
        # ---------- calibration ----------
        note("phase", "calibration — steering should SWEEP")
        r1 = await w(client, vm(0, 0, 0x10), "calib1", wait=1.6)
        r2 = await w(client, vm(0, 0, 0x08), "calib2", wait=2.6)
        await w(client, VM_STATE_REQ, "vm_state_postcalib", wait=0.6)
        note("vm_state_postcalib", vm_state["name"] + " " + str(vm_state["raw"]))

        # ---------- drive sanity: lights ON (should ACK now) ----------
        note("phase", "drive sanity: lights on/off")
